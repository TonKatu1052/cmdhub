...
import { CommandNode, NodeType } from './types';
...
export function removeNodeById(nodes: CommandNode[], id: string): CommandNode[] {
  return nodes.filter(n => n.id !== id).map(n => {
    if (n.type === 'folder' && n.list) { n.list = removeNodeById(n.list, id); }
    return n;
  });
}
...
export function replaceNodeInTree(nodes: CommandNode[], target: CommandNode): CommandNode[] {
  return nodes.map(n => {
    ...
    if (n.type === 'folder' && n.list) { n.list = replaceNodeInTree(n.list, target); }
    return n;
  });
}