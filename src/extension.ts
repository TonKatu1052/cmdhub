...
import type { CommandNode, TerminalType } from './types';
import { addNode, createNode, removeNodeById, replaceNodeInTree } from './node';
...
export function activate(context: vscode.ExtensionContext) {
    ...
    context.subscriptions.push(
        ...
        vscode.commands.registerCommand('cmdhub.edit', async (targetNode?: CommandNode) => {
            ...
            const nodes = replaceNodeInTree(await storage.read(), targetNode);
        ...
        vscode.commands.registerCommand('cmdhub.delete', async (targetNode?: CommandNode) => {
            ...
            const nodes = removeNodeById(await storage.read(), targetNode.id);

            await storage.write(nodes);
            provider.refresh();
        }),

        vscode.commands.registerCommand('cmdhub.run', (targetNode?: CommandNode) => {
        ...
        vscode.commands.registerCommand('cmdhub.setToTerminal', (targetNode?: CommandNode) => {
            if (!targetNode || targetNode.type !== 'command') {
                return;
            }
    ...
    async function addCommand(targetNode?: CommandNode) {
        ...
        const nodes = addNode(await storage.read(), targetNode, newNode);
